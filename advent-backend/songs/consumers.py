...
# ── Choir / Church communities ───────────────────────────────────────────────
def community_channel(kind, community_id):
    return f'community_{kind}_{community_id}'


def broadcast_community_message(kind, community_id, message_data):
    layer = get_channel_layer()
    if not layer:
        return
    async_to_sync(layer.group_send)(
...
def broadcast_community_deleted(kind, community_id, message_id):
    layer = get_channel_layer()
    if not layer:
        return
    async_to_sync(layer.group_send)(
        community_channel(kind, community_id),
        {'type': 'chat_deleted', 'id': message_id},
    )
...
class CommunityChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.kind not in ('choir', 'church'):
            await self.close(code=4400)
            return
        self.group_name = community_channel(self.kind, self.cid)
    ...
    @database_sync_to_async
    def _can_access(self):
        from songs.models import Choir, Church, ChoirMembership, ChurchMembership
        if self.kind == 'choir':
            Model, Membership, fk = Choir, ChoirMembership, 'choir'
        else:
            Model, Membership, fk = Church, ChurchMembership, 'church'
        obj = Model.objects.filter(pk=self.cid).first()
        if not obj:
            return False
        if getattr(self.user, 'is_super_admin', False) or obj.created_by_id == self.user.id:
            return True
        return Membership.objects.filter(**{fk: obj, 'user': self.user}).exists()
