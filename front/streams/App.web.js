...
function NotAuthorized() {
  ...
  // A non-admin has no business in the console. Show a neutral message — no
  // account internals (is_staff / admin_role / capabilities) that could hint at
  // how the gate works — and sign them out automatically after a short beat so
  // they can read it. Root then renders the login screen.
  useEffect(() => {
    const t = setTimeout(() => { logout(); }, 2000);
  ...
  return (
    <View style={styles.centered}>
      <View style={styles.loginCard}>
        ...
        <Text style={styles.muted}>This account doesn’t have admin access. Signing you out…</Text>
        <TouchableOpacity style={styles.primaryBtn} onPress={logout} activeOpacity={0.85}>
          <Text style={styles.primaryBtnText}>Sign out now</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
