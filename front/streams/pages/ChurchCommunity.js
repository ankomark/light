            ...
            <View style={[styles.bubble, mine ? styles.bubbleMine : styles.bubbleOther, isImg && styles.bubbleImage, sending && styles.bubbleSending]}>
              ...
              {item.message_type === 'image' && !!item.attachment && (
                <TouchableOpacity activeOpacity={0.9} onPress={() => (sending ? null : onOpenImage(item.attachment))}>
                  <Image
                    source={{ uri: cldThumb(item.attachment) }}
...
const ChurchCommunity = ({ navigation, route }) => {
  ...
  const [receiptsMsg, setReceiptsMsg] = useState(null);
  ...
  useEffect(() => {
    ...
    const sock = createCommunitySocket('church', churchId, {
      onMessage: (msg) => {
        ...
        markChurchRead(churchId).catch(() => {}); // I'm here reading it → keep receipts fresh
      },
      onEdited: (msg) => {
        if (!msg?.id) return;
  ...
  const submitEdit = async () => {
    ...
    try {
      const updated = await editChurchMessage(churchId, m.id, body);
      setMessages((prev) => prev.map((x) => (x.id === updated.id ? { ...x, ...updated } : x)));
    } catch (e) {
      Alert.alert(t('community.church'), e?.response?.data?.error || t('community.editFailed'));
    }
  };
  ...
  const pin = async (m) => {
    ...
    setPinned(preview);
    try { await pinChurchMessage(churchId, m.id); } catch { setPinned(null); Alert.alert(t('community.church'), t('community.pinFailed')); }
  };
  ...
  const unpin = () => {
    Alert.alert(t('community.unpinTitle'), t('community.unpinConfirm'), [
      ...
      { text: t('community.unpin'), onPress: async () => {
        const prev = pinned;
        setPinned(null);
        try { await unpinChurchMessage(churchId); } catch { setPinned(prev); }
      } },
    ]);
  };
  ...
  const openReceipts = async (m) => {
    ...
    setReceiptsMsg(m);
    setReceipts(null);
    try { setReceipts(await fetchChurchReceipts(churchId, m.id)); }
  ...
  const jumpToMessage = async (messageId) => {
    setShowSearch(false);
    try {
      const res = await fetchChurchContext(churchId, messageId);
      const list = (res?.results || []).slice().reverse();
      if (!list.length) return;
      list.forEach((m) => seenIdsRef.current.add(m.id));
      setMessages(list);
      setHasMore(!!res?.has_earlier);
      setInContext(true);
    } catch { Alert.alert(t('community.church'), t('community.jumpFailed')); }
  };
  ...
  const runMessageSearch = useCallback(async (text) => {
    ...
    setSearchBusy(true);
    try {
      const res = await searchChurchMessages(churchId, text.trim());
          ...
          <FlatList
            ...
            renderItem={({ item: m }) => (
              <TouchableOpacity style={styles.searchResult} onPress={() => jumpToMessage(m.id)} activeOpacity={0.8}>
                <Image source={m.sender?.profile_picture ? { uri: m.sender.profile_picture } : DEFAULT_AVATAR} defaultSource={DEFAULT_AVATAR} style={styles.memberAvatar} />
                <View style={{ flex: 1 }}>
