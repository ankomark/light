...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, Alert, ActivityIndicator, KeyboardAvoidingView, Platform, Image, Modal } from 'react-native';
...
const GroupForm = ({ navigation, route }) => {
  const { group: existingGroup, onCreate } = route.params || {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await updateGroup(existingGroup.slug, formData); // Use slug instead of id
        route.params?.onSubmit?.(response);
      } else {
        response = await createGroup(formData);
        route.params?.onCreate?.(response);
      }

      Alert.alert('Success', isEditMode ? 'Group updated successfully' : 'Group created successfully');
      navigation.goBack();
      ...
      Alert.alert(
        'Error',
        error.detail || error.message || 'Failed to save group. Please try again.'
      );
    } finally {
      setIsLoading(false);
    }
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              setIsLoading(true);
              await deleteGroup(existingGroup.slug); // Use slug for deletion
              route.params?.onDelete?.();
              navigation.goBack();
            } catch (error) {
              console.error('Delete error:', error);
              Alert.alert('Error', error.detail || 'Failed to delete group');
            } finally {
              setIsLoading(false);
            }
          }
        ...
        <Modal transparent={true} animationType="fade" visible={isLoading}>
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <Text style={styles.modalText}>Saving group...</Text>
            </View>
          </View>
        </Modal>
      )}
...
// Styles remain unchanged (omitted for brevity, but use the same styles as provided)
