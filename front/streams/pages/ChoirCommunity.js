...
import {
  ...
  setChoirModerator, fetchChoirAuditLog, fetchChoirMedia,
} from '../services/api';
...
const ChoirCommunity = ({ navigation, route }) => {
  ...
  const loadCommunity = useCallback(async () => {
    ...
    const msgsP = fetchChoirMessages(choirId).catch(() => null);
    try {
  ...
  useEffect(() => {
    ...
    const sock = createCommunitySocket('choir', choirId, {
      onMessage: (msg) => {
  ...
  }, [isMember, choirId, currentUser?.id, markUserTyping]);
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        const res = await fetchChoirMessages(choirId);
  ...
  const toggleModerator = (m) => {
    ...
    setChoirModerator(choirId, m.user.id, next).catch(() => {
      setMembers((p) => p.map((x) => (x.id === m.id ? { ...x, is_moderator: m.is_moderator } : x)));
      Alert.alert(t('community.choir'), t('community.moderatorFailed'));
    });
  };
  ...
  const openAudit = async () => {
    ...
    setAuditBusy(true);
    try {
      const res = await fetchChoirAuditLog(choirId);
  ...
  const openMedia = async () => {
    ...
    setMediaBusy(true);
    try {
      const res = await fetchChoirMedia(choirId);
